...
"""
This method builds the training/testing data for the poker data set. Returns the limit# of examples from the poker data file.
"""
def getNNExtraData(fileString="datasets/poker-hand-training-true.data", limit=30000):
    examples=[]
    data = open(fileString)
    lineNum = 0
    for line in data:
        ...
        count=1
        lineNum+=1
        for attr in re.split(',' , line):
            attr=float(attr)
            if count==11 :
                outputs.append(attr/9.0)        # this is the output, value can be anything from 0-9 (0 being the worst hand) , divided by 9.
            elif count%2==0 :
                inputs.append((attr-1)/12.0)        # this is the rank of the card, value can be anything from 0-12 (0 being Ace) , divided by 12.
            else: 
                inputs.append((attr-1)/3.0)         # this is the suit of the card, value can be anything between 0-3 (0 being Hearts) , divided by 3.
            count +=1
...
"""
builds the examples for the testing and training data sets. 
"""
def buildExamplesFromExtraData():
    pokerDataTrainList = getNNExtraData("datasets/poker-hand-training-true.data",26000)
    pokerDataTestList = getNNExtraData("datasets/poker-hand-testing.data",250)
