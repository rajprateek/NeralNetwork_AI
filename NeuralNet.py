...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        """YOUR CODE"""
        newinActs = inActs[:];
        newinActs.insert(0,1);
        weights = self.getWeightedSum(newinActs)
    ...
    def sigmoidDeriv(self, value):
        ...
        return (exp(value)/((1+exp(value))*(1+exp(value))))
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for j in range(0,len(newlist)):
            currentWeight = self.weights[j]
            updatedWeight = currentWeight+(alpha*newlist[j]*delta)
            self.weights[j] = updatedWeight
            change = updatedWeight-currentWeight
            modifiedList.append(abs(change))
        totalModification = sum(modifiedList)
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        prevLayer  = inActs[:]
        ans = []
        ans.append(prevLayer)
        for i in range(0,self.numHiddenLayers):
            currentLayer = []
            for hLayer in self.hiddenLayers[i]:
                currentLayer.append(hLayer.sigmoidActivation(prevLayer[:]))
            ans.append(currentLayer)
            prevLayer = currentLayer[:]
        last = []
        for j in self.outputLayer:
            last.append(j.sigmoidActivation(prevLayer[:]))
        ans.append(last)
        return ans
